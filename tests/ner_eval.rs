...
fn evaluate_ner_model_against_corpus() {
    ...
    let (model, tokenizer, id2label): (String, String, Vec<String>) =
        if let Some(repo) = env("NER_MODEL_REPO") {
            ...
            let spec = HfModelSpec {
                ...
                tokenizer_file: env("NER_TOKENIZER_FILE").unwrap_or_else(|| "tokenizer.json".to_string()),
                config_file: env("NER_CONFIG_FILE").unwrap_or_else(|| "config.json".to_string()),
            };
            let rt = tokio::runtime::Runtime::new().expect("tokio runtime");
            let resolved = rt.block_on(spec.resolve()).expect("resolve model via hf-hub");
...
fn tally_counts_duplicates_as_multiset() {
    ...
    assert_eq!((c.tp, c.fn_, c.fp), (1, 1, 0), "one of two duplicates is a miss");
    ...
    tally(&expected, &detected, &mut per_kind);
    let c = per_kind[&PiiKind::Person];
    assert_eq!((c.tp, c.fn_, c.fp), (1, 0, 1), "the extra detection is an FP");
}
