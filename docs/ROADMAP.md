...
- [ ] **Array-content fail-closed gap** (`src/pipeline/privacy.rs`, `mask_content`) — an array `content` element that isn't an object (a bare string/scalar) is silently skipped, so PII inside it is forwarded un-masked. Inconsistent with the milestone's own rule (a top-level bad `content` blocks). **Fix**: fail closed on a non-object array element (or mask bare strings), while still skipping known non-text object parts (`image_url`, …). **Test**: pipeline case with `content: ["…pii…"]` (and a mixed array) asserting block-or-mask, never leak.
- [ ] **Phone international over-match** (`src/pii/recognizers.rs`) — `\+\d{1,3}(?: \d{2,7}){1,4}` greedily absorbs a trailing number group: `"+39 333 0000001 12345"` also masks `12345` (same class as the fixed IBAN over-match). **Fix**: tighten the international arm so it can't swallow an unrelated following number. **Test**: adversarial case asserting the span stops at the phone number.
- [ ] **`Confidence` is write-only** (`src/pii/mod.rs`) — populated on every `PiiEntity` but nothing reads it yet. **Fix**: either consume it minimally now (e.g. a `debug`/`trace` log of `Structural` detections for auditability) or annotate it explicitly as reserved for M2/audit, removing the dead-weight ambiguity. **Test**: only if behaviour changes.
