...
- GLINER-WIN-01…05 — `src/pii/gliner.rs` unit tests (**no model**): `GlinerParams::from_config_json` (fields + published defaults) and `plan_word_windows` (single window, token-budget split with one-word overlap, an oversized single word in its own window, empty).
