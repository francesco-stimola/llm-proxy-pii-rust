...
cargo test --release --test phone_eval  -- --ignored --nocapture --test-threads=1 phone_latency
```

**Run both before cutting a `v*` tag**, and update the figures they print in
`ARCHITECTURE.md` → *What the budget costs* and → *Domestic phone coverage*. Everything each of them
reports that is **build-independent** has already been moved into a guard that runs every time:
...
1. **CodeQL is not the guard of the never-log-a-value invariant — two tests in `cargo test` are,
   and they cover different lines.** `DBG-02` (`tests/log_safety.rs`,
   `crate_logs_carry_placeholders_never_raw_pii`) captures the crate's `trace` logs during a real
   round trip and asserts the masked-body line shows `[EMAIL_1]` and never the raw value: that is
   **PII on the request path**. `CFG-03` covers the **startup line**, where the whole `Config` is
   logged. The distinction is not tidiness — it is [M11-R65](reviews/M11.md#m11-r65): a password in
   `UPSTREAM_BASE_URL` was printed in the proxy's first log line, in every tag ever cut,
   while `DBG-02` stayed green — because the startup line is not the line it reads. **A guard for an
   invariant covers the lines it reads, and saying which ones is part of stating the invariant.**
   Every alert in the code-scanning tab today is about a *test* assertion; treating that tab as
   either guard would swap tests that run on every change for a heuristic nobody runs locally.
...
- **PHONE-OM — `the_realistic_turn_yields_exactly_the_expected_phone_spans` (`tests/phone_overmask.rs`): the over-mask guard, on text nobody curated.** The negative corpus contains only the false positives *we thought of*, and M10 widened the candidate set twice over. So this runs the shipped default over the **M7 latency fixture** — a real ~22 KiB Claude Code turn already in the repo, written for a different purpose — and asserts the `Phone` spans are exactly the expected set (today: **empty**). The fixture moved to `tests/common/m7_turn.rs` so both this (default build) and `m7_latency.rs` (`onnx`-only) use *one* copy. Non-vacuity is asserted three ways: the fixture must still be >20 KB, its **shape** is pinned here (M10-R12 — `m7_latency.rs`'s full shape assertion is `#[ignore]`d *and* `onnx`-gated, so it never runs in CI), and a sibling positive control splices in **one number per shape family**. That last part is the M10-R7 fix and it is load-bearing: with a single trunk-anchored control, deleting both un-anchored families left every assertion green — fewer recognizers cannot find more, so the empty expectation still held — while the two families the guard exists to bound were gone.
  - **Its fixture had a shape, and `PHONE-OM-TOOL` is the second fixture that closes it
    ...
    both became `[PHONE_n]`. *A real-traffic guard bounds its claim to that traffic*: this turn is
    instruction boilerplate, tool schemas and prose, and carries no `tool_result` holding command
    output — which is where a coding agent's digit-shaped text actually lives. Non-vacuity checks
    cannot repair that; only a second fixture can.
- **PHONE-OM-TOOL (M11-R55) — `the_tool_output_turn_yields_exactly_the_expected_phone_spans`
  (`tests/phone_overmask.rs`): the same construction over the shape the M7 turn does not have.**
  `tests/common/tool_output_turn.rs` is six fields of ordinary agent traffic — `ls -l`, `df -h`, a
  `psql` result, `journalctl` lines with public and RFC 1918 addresses, an `ssh` `tool_use.input`,
  and a user message — and **every number in it is a real quantity of its own kind, none of them a
  phone number**, so any `Phone` span it yields is an over-mask by construction.
  **Its expectation is the measured set, not zero, and that is the decision showing up in the
...
- **PHONE-EVAL *(`tests/phone_eval.rs`)* — the measurement that *is* M10's deliverable, and since
  ...
  *897* for a pool that had become 945). **FP is reported per category, never blended** — one rate
  ...
  - **The published table is the harness's own output, asserted (M11-R55).** `docs/ARCHITECTURE.md`
    carries the measurement between `<!-- PHONE-EVAL:BEGIN -->` and `<!-- PHONE-EVAL:END -->`, and
    the test renders the same block from a live run and requires equality — printing the new block
    to paste when it differs. **This is not two lists checking each other** (M11-R53's bookkeeping):
    one side is a measurement of the product, the other is the sentence the operator reads, and
    there is no third copy to drift. It replaces a hand-transcribed table under which `dates 0.180`
    survived four widenings that had made it 0.270.
  - **`#[ignore]`d was the whole finding of [M11-R55](reviews/M11.md#m11-r55), not a footnote about
    it.** This is the **only** precision harness the domestic-phone tier has, and `cargo test` never
    ran it — so the four consecutive M11 widenings of the separator axis (R25 -> R48 -> R51/R52)
    ...
    not"**, so a widening's cost was invisible to the suite by construction.
    **The rule: a change that widens what a recognizer matches must re-run the harness that measures
    what it refuses** — which is now enforced rather than remembered. Cost of enforcing it: ~13 s in
    the debug profile, paid once per suite run.
  - **The pool gained `ips`, `ips10`, `ips192`, `ips172` and `aligned`**, the shapes round 14
    admitted, and the harness's own first assertion now covers them: a pool must be able to *reach*
    a shape family before its 0.000 means "clean" rather than "unmeasured", and dotted-quad and
    2-4-space-aligned text are reachable shapes. They are generated from a **seeded LCG, not
    `rand`** — the rates are asserted against a document, so the pool has to be byte-identical on
    every box. The three private ranges are separate categories rather than one blended "private
    IP" rate, for the same reason the whole table is per-category.
...
- **SEPARATOR-01 (M11-R25) — `every_gap_bearing_recognizer_answers_the_separator_axis`: the third
  ...
  **Closed:** that row now carries `+39.347.1234567`, `+39/347/1234567`, `+39-347-1234567` and
  ...
  behaviour.*
  **The decided limit of this axis, measured and written where it is read (M11-R55).**
  `SEPARATOR_RUN_MAX = 4` and the alphabet `- . / ( )` are both *coverage* choices, and their cost
  ...
  close later: it is the trade, and the number is what makes it a trade rather than an accident.*
...
- **UTF8-01 (M11-R21) — `a_non_ascii_digit_inside_a_value_never_panics_a_validator`: `\d` is
  ...
  **And the residue it does *not* measure is the half of the recognizer set it cannot see
  (M11-R40).** `CASE_ANSWERS` is scoped by `pattern_can_match_a_letter` — right on the case axis,
  inverted on this one — so **12 of the 24 shipped recognizers are outside this guard**: the card,
...
- **DOS-10 (M11-R18) — *letters mixed with digits*, the alphabet every other case in this file holds
  constant.** `a_field_of_lowercase_alphanumeric_groups_is_charged_for_the_case_gate`. DOS-01…03 vary
  field size, DOS-04 entity count, DOS-05 the alphabet (and its bytes are digit groups), DOS-06
  periodicity, DOS-07 field count — **every one of them is digits, `sk-` runs or `a@b.co`.** Not one
  ...
  **It asserts that the work is *charged*, not that it is fast**, and the distinction is the point: a
  ...
  of the file), while what must never come back is the term being invisible to the budget — a
  property of the code. Three assertions: the lowercase body spends units, the **uppercase control**
  spends at least ten times fewer (the gate short-circuits on a span with no lowercase byte *without
  arithmetic*, so a charge firing on both would be a charge on candidate count and would refuse legal
  uppercase bodies for work nobody did), and the charge is **linear** in the body. Non-vacuity first:
  the uppercase body must yield >100 spans, or "spent 0" would mean *nothing to charge* rather than
  *not charged*.
...
- **DOS-BUD (M10-R30) — the measurement, `#[ignore]`d and run on `--release`.** `budget_refusal_line_and_cost` prints where the refusal line falls and what it costs: one field at increasing sizes, one request at increasing *field counts*, a realistic SQL tool result with one phone column, and the same body with the phone tier off. This exists because the published bound was quoted from a constant for three review rounds and was wrong in three ways at once — the wrong unit, a figure 2–4× optimistic, and a claim about "phone-shaped" text that bare 9-digit tokens also satisfied. The number in `MAX_PHONE_VALIDATIONS_PER_REQUEST` now comes from **here**, and its realistic-payload row is what set it: at 50,000 units an ordinary 367 KB database result was refused. Also the source of the ~3 µs per `parse()` figure. **Two rows exist because of what they cost to omit:** the SQL shape at `MAX_BODY_BYTES` in **two legal renderings** (M10-R53/R54 — `347 XXXXXXX` is masked at 16 MiB, the same numbers as `3XX XXX XXXX` are **refused**), and a per-rendering unit table showing the 1-to-29 spread. Twice this harness generalized an availability claim from a single point of its own grid; varying the rendering is what turns *"can real traffic reach the allowance?"* from a guess into a band. **Its SQL rows go through `Vault::mask_all`, not `try_detect`** — measuring one pass is how the published figure came to be per-pass while claiming to be per-field (M10-R30), and after M10-R35 the later passes are charged, which roughly doubles a masking body's spend. **A row added by M11-R60 prints where an *ordinary* body stops being accepted, by size, with the per-kind split** — the `xxd` shape is the one that reaches both spenders at once, and the split says which of them is responsible at each size, which is the same information the refusal message now carries to the client. A final row measures what the budget does **not** bound: 16 MiB with the phone tier off, the unbudgeted linear floor, so the per-request ceiling can be published as *validation + linear work* instead of the validation term alone wearing the word "ceiling".
  ...
  > `national_phone_valid`'s `.any()` short-circuited on accept everywhere in the grid while the code
  > named the other branch as the expensive one. M11-R39 added the verdict axis. Running it then
  > showed the axis that *still* was not varied: the candidate's **group count**. The grid now prints
  > **µs per unit** beside ms per row, over four shapes — and the two axes turn out to do different
  > jobs:
  >
  > - the **verdict** moves how many units a candidate spends (a rejection pays every enabled region),
  >   and the budget's count already tracks that;
  > - the **shape** moves what one unit *costs*, and nothing tracks it: valid `3XX XXX XXXX`
  >   **3.2–4.3 µs**, rejected `0NNNN NNNN` **2.9–3.5**, lowercase `ab12 cd34` **1.3–1.8**, three-group
  >   `0NNN NNNN NNNN` **15.1–21.7**. An ~18× spread, and `~3 µs` sat near its bottom.
  >
  > **The rule, and it is not about phones:** *a complexity guard that reports a rate has to report
  > the numerator and the denominator separately, or it cannot tell "more units" from "dearer units".*
  > With the verdict fixed, ms/row and µs/unit are interchangeable — which is why nothing ever asked
  > whether the unit varied.
  >
  > **And a disagreement this closed, because two rounds read two different rows as the same number
  > (M11-R58).** Round 14 published `3XX XXX XXXX` at *8 -> 9.8 units/row, +22%*; round 15 measured
  > **8.00 on both builds** and could not reproduce it. Both are right about their own row.
  > Re-measured against `ed5214b` in a worktree, the verdict grid's spend is **+9,000 units flat** at
  > 5,000 / 20,000 / 50,000 rows and on both shapes — five rows out of five — so the per-row *ratio*
  > is a constant divided by the row count: 1.8 at 5,000 rows, 0.45 at 20,000, and 0.00 in the bare
  > 20,000-row column grid, which carries no non-phone text at all. Attributed: deleting the `NNN.50`
  > money column from the body at HEAD reproduces `ed5214b`'s figure **exactly** (40,000 at 5,000 rows,
  > 160,000 at 20,000), because round 14's alphabet made `123.50` a `Groups` candidate — and the
  > offset is *constant* rather than per-row because the per-scan memo collapses its 900 distinct
  > values. **Neither number was a per-row cost**, and the phone column's own per-row cost did not
  > move. *A conclusion drawn from one point of a grid is a fact about that point* — M10-R49 / R53 /
...
- **CFG-03 (M11-R65) — `debug_redacts_every_secret_and_leaves_a_plain_base_url_verbatim` + `credentials_in_the_base_url_never_reach_the_startup_log` (`tests/binary_smoke.rs`).** `main` logs the whole configuration at startup, so `Config`'s `Debug` **is** a log line — and `upstream_base_url` was printed whole, immediately above `upstream_api_key: Some("<redacted>")` and in the same struct as extra headers reduced to their names *because a value may be a secret*. A URL may carry `user:password@`, so `UPSTREAM_BASE_URL="https://alice:hunter2@api.openai.com"` put a password in the first line the proxy writes, in every tag ever cut. **Redacted in `Debug` only, never in the stored value** — `proxy.rs` concatenates onto that string and the upstream still has to receive the credentials. **Two halves:** the unit half asserts the *rendered* `Debug` (a redaction the impl stopped calling would satisfy a test of the function), and the runtime half drives the real `.exe` and reads what it logged, because nothing in-process can see whether `main` logs that `Debug` at all — measured: teach `main` to log `upstream = %config.upstream_base_url` beside it and the unit half stays **green** while the runtime half goes red. The half that guards the *legitimate* path is the third assertion: a credential-free URL must appear **verbatim**, which is what an operator reads the startup line for, and what a too-eager redaction breaks. A value outside the shape `CFG-02` admits is withheld as `<unprintable>` rather than guessed at — `alice:hunter2@not-a-url` *parses* (scheme `alice`, no authority), so a userinfo-only check prints the password, which is how the first draft of this fix leaked.
  > **The decided limit, so the guard is not read as more than it is:** only *userinfo* is redacted. A secret written into the **query or path** of the base URL (`…?api-key=…`) is still printed, because the rule is that a URL without credentials appears exactly as configured — the startup line an operator checks. Widening past userinfo would mean printing scheme and host only, and that trade was decided the other way.
