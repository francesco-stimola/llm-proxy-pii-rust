...
- LOC-16 — **Union-merge: no abandoned bytes (M4-R10 / M4-R11)** — a *partial* overlap must mask **both** spans, not pick a winner:
  ...
  - `a_span_deleted_by_the_containment_gate_is_never_stranded` (recognizers): a card/secret **enclosed** by an email, with a phone partially overlapping that email — the enclosed span must still be covered (under the old gate it was *deleted* before the priority sort, and when the enclosing email then lost, it was masked by *nothing*). *(The test name is now historical — M4-R15 removed the gate; nothing is deleted, so nothing can be stranded. Tracked for a rename in M4-R18.)*
