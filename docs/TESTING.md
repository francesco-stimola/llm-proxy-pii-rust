...
- **KIND-01 (M11-R3) — `all_lists_every_variant_in_order`: the list the per-variant guards are
  driven from cannot fall behind the enum.** M11 added an eleventh kind and a review round showed a
  *twelfth* could be added with the whole suite green: every per-variant guard in the codebase
  watched a hand-typed list, so nothing forced a new variant to be considered anywhere.
  `PiiKind::ALL` is the chokepoint — but a hand-typed `ALL` would have the same defect one level
  up, so this rebuilds the list **without reading it**, by walking a successor chain whose `match`
  the compiler checks for exhaustiveness. A new variant is a compile error there, its author has to
  place it in the chain, and the walk then reproduces `ALL` independently; the comparison is what
  fails if `ALL` was not updated. A variant wired to itself or into the middle of the chain is
  caught by the visited check rather than hanging the test.
- **KIND-02 (M11-R3) — `every_kind_round_trips_through_its_label`.** `PiiKind::label` names a
  variant and `from_label` is its inverse, ending in `_ => return None` — so a kind could be added
  to one and not the other with nothing failing. That is not cosmetic: `from_label` gates
