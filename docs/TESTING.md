...
- **PHONE-NAT-10 (M10-R2) — `the_length_gate_never_rejects_a_number_a_region_accepts`.** The digit-count gate sits in front of a privacy decision, so "it makes things faster" is not sufficient: it must be incapable of rejecting a candidate some region would take. Checks the derivation end to end — the metadata really yields lengths (an empty mask silently masks *nothing*, which is how the first cut failed), the +1 trunk allowance covers every enabled region's actual prefix, and every corpus positive clears the gate.
