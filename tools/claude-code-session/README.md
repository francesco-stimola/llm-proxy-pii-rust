...
| `fixtures/customer-lookup.sql` | CC-09 | `SELECT … FROM DUAL` returning every structured category — no schema or real data needed |
| `scratch/` | CC-04 | gitignored. Point tool-*writing* scenarios here: with the de-mask skipped, a tool call writes `[EMAIL_1]` into a file — worth seeing, never worth committing |
